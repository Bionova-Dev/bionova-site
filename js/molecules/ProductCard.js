...
const ProductCard = ({ product, onAddToCart, onClick }) => {
  ...
  return (
    <CardWrapper
      ...
      className="bg-gray-50 rounded-[3rem] p-8 sm:p-10 flex flex-col h-full items-center text-center group border border-gray-100 hover:shadow-2xl hover:border-medical-blue/20 transition-shadow duration-500 relative cursor-pointer"
    >
      {/* Dynamic Product Badge (Premium, Économisez 10%, etc.) */}
      {product.badge && (
        <div className="absolute top-6 right-6 bg-[#be123c] text-white text-[10px] font-black px-4 py-2 rounded-full shadow-lg tracking-wider uppercase z-20">
          {product.badge}
        </div>
      )}
      ...
      {product.image2 ? (
        <div className="w-48 h-48 sm:w-56 sm:h-56 mb-10 relative flex items-center justify-center p-2 gap-2">
          <img
            src={product.image}
            alt={product.name}
