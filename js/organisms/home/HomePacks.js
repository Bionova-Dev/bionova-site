      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row justify-between items-end mb-16">
          <div className="md:w-2/3">
            <h2 className="text-sm text-[#e4002b] font-bold tracking-widest uppercase mb-3">Offres Limitées</h2>
        ...
        <div className="grid grid-cols-1 gap-12">
          {packs.map(pack => (
            <div key={pack.id} onClick={() => onProductClick(pack)} className="bg-white rounded-2xl sm:rounded-[3rem] lg:rounded-[4rem] overflow-hidden flex flex-col lg:flex-row items-center gold-border hover:shadow-2xl transition-all duration-500 relative group cursor-pointer">
              ...
              <div className="lg:w-1/2 p-12 lg:p-20 flex flex-col justify-center">
                <div className="inline-block px-4 py-1.5 bg-rose-50 text-[#e4002b] rounded-full text-[10px] font-bold tracking-widest uppercase mb-6 self-start">Nutricosmétique Premium</div>
                <h3 className="font-display text-xl sm:text-2xl lg:text-4xl xl:text-5xl font-black text-gray-900 mb-2 group-hover:text-[#e4002b] transition-colors">{pack.name}</h3>
                ...
                <div className="flex items-end space-x-6 mb-12">
                  ...
                  <div><p className="text-sm text-[#e4002b] font-bold uppercase tracking-widest mb-1">Prix Pack</p><p className="text-3xl sm:text-4xl lg:text-5xl font-black text-[#e4002b]">{pack.price.toFixed(2)} <span className="text-xl">DT</span></p></div>
                </div>
                <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); onAddToCart(pack); }} className="w-full py-4 px-6 bg-[#e4002b] text-white text-base sm:py-6 sm:px-10 sm:text-lg font-bold rounded-2xl shadow-xl hover:shadow-2xl transition-all transform hover:-translate-y-1 text-center cursor-pointer">Ajouter le pack au panier</button>
              </div>
            </div>
          ))}
        </div>
