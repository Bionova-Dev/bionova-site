    ...
    <section className="py-28 bg-white border-t border-gray-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-20">
          <h2 className="text-sm text-[#e4002b] font-bold tracking-widest uppercase mb-3">Témoignages</h2>
          <p className="font-display text-4xl font-extrabold text-gray-900 sm:text-5xl">Ce que disent nos clients</p>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {testimonials.map((t, i) => (
            <div key={i} className="bg-gray-50 rounded-[2.5rem] p-6 sm:p-8 lg:p-10 border border-gray-100 hover:shadow-xl transition-all duration-500 relative group">
              <div className="absolute -top-4 left-10 text-6xl text-[#e4002b]/10 font-display font-black leading-none">"</div>
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="px-3 py-1 bg-[#e4002b]/5 text-[#e4002b] text-xs font-bold rounded-full">{t.product}</span>
              </div>
            </div>
          ))}
        </div>
