...
const App = () => {
  ...
  const updateMenuHighlights = (page) => {
    ...
    document.querySelectorAll('#main-header .nav-item').forEach(el => {
      if (el.textContent.trim() === targetLabel) {
        el.classList.add('text-[#be123c]', 'border-[#be123c]');
        el.classList.remove('text-black', 'border-transparent');
      } else {
        el.classList.remove('text-[#be123c]', 'border-[#be123c]');
    ...
    document.querySelectorAll('#mobile-menu a').forEach(el => {
      if (el.textContent.trim() === targetLabel) {
        el.classList.add('text-[#be123c]', 'bg-gray-50');
        el.classList.remove('text-black');
      } else {
        el.classList.remove('text-[#be123c]', 'bg-gray-50');
  ...
  const handleCategoryChange = (categoryId) => {
    ...
    const targetUrl = categoryId === 'all' 
      ? 'https://bionova.tn/boutique/' 
      : `https://bionova.tn/categorie/${categoryId}/`;
  ...
  const handleAddToCart = (product, redirect = false) => {
    ...
    fetch(addUrl, { credentials: 'same-origin' })
      .then(() => {
        ...
        // Redirection vers le panier pour les packs ou si forcé
        if (redirect || product.type === 'pack') {
          window.location.href = window.WC_CART_URL || '/panier/';
        } else {
          // Toast notification pour les ajouts standards
      ...
      .catch(err => console.error("Erreur d'ajout au panier:", err));
  };
