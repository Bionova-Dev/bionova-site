  ...
  <script type="text/babel">
    ...
    const Navbar = ({ cartItemCount, currentPage, onNavigate }) => {
      ...
      const navLinkClass = (page, isMobile = false) => {
        ...
        return `${baseStyles} transition-all duration-300 cursor-pointer ${currentPage === page ? activeColor : `${baseColor} border-transparent hover:text-black`}`;
      };
            ...
            <nav className="max-w-[1800px] mx-auto px-4 lg:px-12 w-full h-full" aria-label="Navigation principale">
              <div className="flex justify-between items-center h-full gap-4 lg:gap-8">
                ...
                <div className="flex items-center space-x-2 sm:space-x-6 ml-auto">
                  <button onClick={() => setIsSearchOpen(true)} className="p-2 sm:p-3 rounded-2xl text-black hover:text-black hover:bg-gray-100 transition-all group" title="Rechercher">
                    <SearchIcon className="h-6 w-6 lg:h-7 lg:w-7 group-hover:scale-110 transition-transform" />
                  </button>
                  <button onClick={() => onNavigate('login')} className="hidden sm:block p-3 rounded-2xl text-black hover:text-black hover:bg-gray-100 transition-all group" title="Mon compte">
                    <UserIcon className="h-7 w-7 group-hover:scale-110 transition-transform" />
                  </button>
                  <a href={WC_CART_URL} className="relative p-3 sm:p-4 rounded-2xl bg-black text-white hover:bg-gray-800 transition-all group shadow-lg hover:shadow-xl" title="Voir le panier">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-20">
            ...
            <div className="text-center md:text-left">
              <h4 className="font-display text-xl font-bold text-black uppercase tracking-widest mb-8">Boutique</h4>
              <ul className="space-y-4">
                <li><a href="#products" className="text-gray-600 hover:text-black transition-colors font-bold text-sm">Tous les produits</a></li>
                <li><a href="#products" className="text-gray-600 hover:text-black transition-colors font-bold text-sm">Packs Synergie</a></li>
                <li><a href="#products" className="text-gray-600 hover:text-black transition-colors font-bold text-sm">Nouveautés</a></li>
                <li><a href="#products" className="text-gray-600 hover:text-black transition-colors font-bold text-sm">Meilleures Ventes</a></li>
              </ul>
            </div>
